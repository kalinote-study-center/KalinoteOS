...
void asm_inthandler07(void);							// 07号中断，FPU
...
int *inthandler07(int *esp);																		// FPU中断
...
#define	IDE0_LBA_LOW			0x173								// IDE控制器0读写扇区号码(LBA低8位)
#define	IDE0_LBA_MID			0x174								// IDE控制器0读写柱面低8位(LBA8-15位)
#define	IDE0_LBA_HIGH			0x175								// IDE控制器0读写柱面高2位(LBA16-23位)
#define	IDE0_DEVICE				0x176								// IDE控制器0读写磁盘号和磁头号(包含LBA24-27位，第四位用于指定主盘或从盘，第六位设置是否启动LBA方式，第五位和第七位固定为1)
#define	IDE0_STATUS				0x177								// IDE控制器0读操作后的状态
#define	IDE1_DATA				0x1f0								// IDE控制器1数据端口，用来传送读/写的数据(其内容是正在传输的一个字节的数据)
#define	IDE1_ERRCODE			0x1f1								// IDE控制器1错误码端口，用来读取错误码
#define	IDE1_NSECTOR			0x1f2								// IDE控制器1读写的扇区数量
#define	IDE1_LBA_LOW			0x1f3								// IDE控制器1读写扇区号码(LBA低8位)
#define	IDE1_LBA_MID			0x1f4								// IDE控制器1读写柱面低8位(LBA8-15位)
#define	IDE1_LBA_HIGH			0x1f5								// IDE控制器1读写柱面高2位(LBA16-23位)
#define	IDE1_DEVICE				0x1f6								// IDE控制器1读写磁盘号和磁头号(包含LBA24-27位)
#define	IDE1_STATUS				0x1f7								// IDE控制器1读操作后的状态
#define	IDE_HD_NUM_ADDR			0x475								// IDE硬盘数量信息缓存地址(通过BIOS)
...
#define	STATUS_DSC				0x10								// 磁盘驱动器搜索完成
#define	STATUS_DRQ				0x08								// 扇区缓冲区没有准备好
#define	STATUS_CORR				0x04								// 是否正确读取磁盘
#define	STATUS_IDX				0x02								// 磁盘旋转一周
#define	STATUS_ERR				0x01								// 上一个命令因错误而结束
#define REG_DEV_CTRL			0x3f6								// 0x3f6是ATA备用状态寄存器
