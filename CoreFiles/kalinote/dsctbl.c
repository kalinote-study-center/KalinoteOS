...
void init_gdtidt(void)
{
	...
	/* IDT的设定(中断注册) */
	set_gatedesc(idt + 0x0c, (int) asm_inthandler0c, 2 << 3, AR_INTGATE32);						/* 这里的2<<3表示中断段号为2，左移3位是因为最低3位有其他作用 */
	...
	set_gatedesc(idt + 0x40, (int) asm_kal_api,      2 << 3, AR_INTGATE32 + 0x60);				/* 应用程序API中断 */
