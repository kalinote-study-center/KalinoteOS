...
/*
* 有些时候可能会用到嵌入式汇编代码，现在在这里对其格式进行说明：
* asm("汇编语句"
*	: 输出寄存器
*	: 输入寄存器
*	: 会被修改的寄存器)
...
void asm_inthandler_device_not_available(void);			// 07号中断，数学协处理器不存在
void asm_inthandler_stack_segment(void);				// 0c号中断，用于处理栈异常
void asm_inthandler_general_protection(void);			// 0d号中断，用于处理异常程序
...
void reset_cpu(void);																				// 复位CPU(8042重启)
...
void sheet_refreshall(struct SHEET *sht, int bx0, int by0, int bx1, int by1);						// (未完成)从子图层到父图层全部刷新
...
int signal(int signum, int handler, int restorer);													// 为指定信号设置信号处理程序
int sigaction(int signum, struct SIGACTION *action, struct SIGACTION *oldaction);					// 改变进程收到信号时的操作
void do_signal(int sig_num, int eax, int ebx,
	int ecx, int edx, int fs, int es, int ds,
	int eip, int cs, int eflags, unsigned int *esp, int ss);										// (未完成)信号处理程序(TODO)
...
struct TASK {
	...
	struct SEGMENT_DESCRIPTOR ldt[2];						// LDT描述符
...
int *inthandler_device_not_available(int *esp);														// 数学协处理器不存在
...
struct TEXTBOX {
	int x,y;					/* 图层相对坐标 */
	int height,width;			/* 高宽 */
	struct SHEET *sht;			/* 输入框的图层 */
	char flags;					/* 输入框状态标识(bitmap,低1位标识是否激活，低2位表示是否可视，低3位标识是否多行) */
	int cur_x, cur_y, cur_c;	/* 光标位置(相对)及状态(cur_c) */
	// char *buf;				/* (和sheet中的一样，所以不用单独设置)保存输入内容的缓冲区 */
	int char_num;				/* 最大字符数量 */
	int back_color;				/* 背景颜色 */
};
void make_textbox_old(struct SHEET *sht, int x0, int y0, int sx, int sy, int c);						// (旧版)生成输入框
struct TEXTBOX *make_textbox(struct SHTCTL *shtctl, int x0, int y0, int height,
	int width, int back_color, unsigned int *buf, int char_num);										// 生成输入框
void show_textbox(struct TEXTBOX *textbox);																// 显示textbox
...
/* exit.c(任务结束时的处理) */
void do_exit(int *esp);										// (未完成)退出时相关处理
void tell_father(int pid);									// (未完成)向父进程发送SIGCHLD信号
int task_kill(int pid, int sig);							// (未完成)向进程(任务)或组发送信号
void task_kill_session(void);								// 终止会话

/* fork.c(复制进程) */
int copy_mem(int task_num, struct TASK *task);									// (未完成)进程复制时的相关内存复制
int copy_process(unsigned int fsize, unsigned int segsiz);						// (未完成)复制进程
...
int *inthandler_stack_segment(int *esp);															// 0c号中断，用于处理栈异常
int *inthandler_general_protection(int *esp);														// 0d号中断，用于处理一般异常
...
void list_insert_before(struct list_elem* before, struct list_elem* elem);		// (未完成)将链表元素elem插入到before之前
void list_push(struct list* plist, struct list_elem* elem);						// (未完成)添加元素到列表队首,类似栈push操作
void list_append(struct list* plist, struct list_elem* elem);					// (未完成)追加元素到链表队尾,类似队列的先进先出操作
void list_remove(struct list_elem* pelem);										// (未完成)使元素pelem脱离链表
struct list_elem* list_pop(struct list* plist);									// (未完成)将链表第一个元素弹出并返回,类似栈的pop操作
int elem_find(struct list* plist, struct list_elem* obj_elem);					// (未完成)从链表中查找元素obj_elem,成功时返回所在位置,失败时返回-1
int list_empty(struct list* plist);												// (未完成)判断链表是否为空,空时返回1,否则返回0
...
int cpu_64_check(void);													/* (未完成)检测CPU是否支持64位 */

/* io.c(输入输出控制) */
#define IO_READ		1	/* 载入 */
#define IO_WRITE	2	/* 写入 */
#define IO_EXIT		3	/* 处理完毕 */
...
void fdc_tinit(struct TASK *task);					// 初始化FDC任务
void fdc_dmainit(void);
void fdc_req(char mode, char cyl, char head, char sect, char send);
int fdc_reqstat(void);
void fdc_init(void);								// 初始化FDC
void fdc_initwait(int wait);						// 初始化过程中的等待程序
void fdc_sendcmd(int data);							// 向FDC发送指令
void fdc_setdma(void);								// 设置DMA
void fdc_sethead(void);								// 设置head
void fdc_setcmd(void);								// 设置命令模式
char fdc_getrstatsub(void);							// 获取寄存器状态子程序
void fdc_getint(void);								// 获取中断状态
void fdc_getrstat(void);							// 获取寄存器状态
int fdc_rdwri(void);								// 控制读写
void fdc_task(void);								// FDC驱动任务程序
...
/* 以下代码用于测试(调试)，在系统正常使用时(多数情况)不会用到 */
