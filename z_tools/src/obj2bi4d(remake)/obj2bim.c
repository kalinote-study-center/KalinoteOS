...
#define	MAXSECTION		64	/* ?个.obj文件的最大?数 */

#define NO_WARN			1	/* 用于消除警告的无用代? */
...
struct LABELSTR {
	...
	struct OBJFILESTR *name_obj; /* 指向本地.obj的指?。如果是public的?NULL */
	struct OBJFILESTR *def_obj; /* 所属?象文件 */
	unsigned int offset;
};
...
int main(int argc, UCHAR **argv)
{
	...
	int section_param[12]; /* 前四个是代?，后四个是数据 */
	struct LABELSTR *label;
